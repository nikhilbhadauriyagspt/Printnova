...
exports.getSEO = async (req, res) => {
    try {
        const website_id = req.query.website_id || 1;
        const [content] = await db.query('SELECT * FROM seo_content WHERE website_id = ?', [website_id]);
        res.json(content);
    } catch (error) {
...
exports.getSEOByPage = async (req, res) => {
    ...
    try {
        ...
        // 4. Otherwise, it's a Static Page (home, about, etc.)
            ...
            [content] = await db.query('SELECT meta_title, meta_description, meta_keywords FROM seo_content WHERE page_name = ? AND website_id = ?', [page, website_id]);
        }
...
exports.updateSEO = async (req, res) => {
    const { page_name, meta_title, meta_description, meta_keywords, website_id } = req.body;
    const siteId = website_id || 1;
    try {
        const [existing] = await db.query('SELECT id FROM seo_content WHERE page_name = ? AND website_id = ?', [page_name, siteId]);
        
        if (existing.length > 0) {
            await db.query(
                'UPDATE seo_content SET meta_title = ?, meta_description = ?, meta_keywords = ? WHERE page_name = ? AND website_id = ?',
                [meta_title, meta_description, meta_keywords, page_name, siteId]
            );
        } else {
            await db.query(
                'INSERT INTO seo_content (page_name, meta_title, meta_description, meta_keywords, website_id) VALUES (?, ?, ?, ?, ?)',
                [page_name, meta_title, meta_description, meta_keywords, siteId]
            );
        }
